...
const sources = [
  ...
  { name: "גאדג׳טי — אפל", category: "אפל", feeds: ["https://www.gadgety.co.il/tag/apple/"], limit: 12 },
...
function parseI24(body, source, base) {
  const seen = new Set(), items = [];
  for (const match of body.matchAll(/<a\b([^>]*href=["'][^"']+\/he\/news\/news\/[^"']+\/artc-[^"']+["'][^>]*)>([\s\S]*?)<\/a>/gi)) {
    const href = /href=["']([^"']+)/i.exec(match[1])?.[1];
    const url = absolute(href, base);
...
function parseGadgety(body, source, base) {
  const seen = new Set(), items = [];
  for (const match of body.matchAll(/<a\b([^>]*href=["'][^"']+["'][^>]*)>([\s\S]*?)<\/a>/gi)) {
    const href = /href=["']([^"']+)/i.exec(match[1])?.[1];
    const url = absolute(href, base);
    const title = clean(match[2]);
    if (!/^https:\/\/www\.gadgety\.co\.il\/\d{4,}\/[^/]+\/?(?:[?#].*)?$/i.test(url) || title.length < 24 || title.length > 190 || seen.has(url)) continue;
    const image = absolute(match[2].match(/<img\b[^>]*(?:src|data-src)=["']([^"']+)/i)?.[1], base);
    seen.add(url);
    items.push({ category: source.category, source: source.name, title, summary: "", url, image, publishedAt: "" });
  }
  return items.slice(0, source.limit || 12);
}
...
async function readSource(source, isRotter = false) {
  for (const url of source.feeds) {
    try {
      ...
      const items = isRotter ? parseRotter(body, url) : source.name === "i24NEWS" ? parseI24(body, source, url) : source.name === "Let's AI" ? parseLetsAI(body, source, url) : source.name === "גאדג׳טי — אפל" ? parseGadgety(body, source, url) : source.name.startsWith("ערוץ הספורט") ? parseSport5(body, source, url) : /<(rss|feed)\b/i.test(body) ? parseRss(body, source, url) : parseHtml(body, source, url);
